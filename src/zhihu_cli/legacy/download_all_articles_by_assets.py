...
class ArticleDownloadPipeline:
    ...
    def __init__(self, assets_file: str | None = None, output_dir: str | None = None):
        ...
        # 统计信息
        self.stats = {"total": 0, "success": 0, "failed": 0, "skipped": 0, "failed_urls": []}
...
def save_headers(headers: dict[str, str]) -> None:
    ...
    """
    # 移除可能敏感的信息
    safe_headers = {k: v for k, v in headers.items() if k.lower() not in ["cookie", "authorization", "x-zse-93"]}
...
def main() -> None:
    ...
    parser.add_argument("--quick", "-q", action="store_true", help="快速模式，自动使用已保存的请求头")

    args = parser.parse_args()

    if args.quick:
        pipeline = QuickDownloadPipeline(assets_file=args.assets_file, output_dir=args.output_dir)
        pipeline.run_auto(delay=args.delay, resume_from=args.resume)
    else:
        pipeline = ArticleDownloadPipeline(assets_file=args.assets_file, output_dir=args.output_dir)
