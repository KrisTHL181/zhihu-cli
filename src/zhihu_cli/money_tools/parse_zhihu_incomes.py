...
def run_task():
    ...
    while current_dt < end_dt:
        ...
        if batch_end < current_dt: batch_end = current_dt
        ...
        try:
            ...
            if resp.status_code == 200 and resp.text.strip():
                ...
                count = 0
                for row in reader:
                    if not row or "日期" in row[0]: continue
                    
                    salt_grains = int(row[1]) if len(row) > 1 else 0
                    yuan = round(salt_grains / 100.0, 2)
                    
                    new_income_data.append({
                        "date": row[0],
                        "income_salt": salt_grains,
                        "income_yuan": yuan
                    })
                    count += 1
        ...
        if batch_end >= end_dt - timedelta(days=1): break
