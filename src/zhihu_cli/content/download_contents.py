...
def extract_metadata_from_html(html_content: str) -> Dict[str, str]:
    ...
    title_pattern = r'<meta\s+itemprop="name"\s+content="([^"]+)"|<h1[^>]*class="[^"]*Post-Title[^"]*"[^>]*>([^<]+)</h1>'
...
def main():
    ...
    # 定义下载子命令/参数
    parser.add_argument('--download-articles', nargs='+', metavar='URL', 
                        help='下载文章页面并转换为 Markdown (支持多个 URL)')
    parser.add_argument('--download-answers', nargs='+', metavar='URL', 
                        help='下载回答页面')
    parser.add_argument('--download-pins', nargs='+', metavar='URL', 
                        help='下载想法页面')
