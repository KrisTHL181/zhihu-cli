...
def plot_analysis():
    try:
        ...
        plt.plot(df['date'], df['ema7'], color='#ff9800', linewidth=2.5, 
                 label='EMA (7-Day Momentum)')
        
        # 3. 线性趋势线 (长期表现)
        plt.plot(df['date'], trend_line, color='red', linestyle='--', linewidth=1.5, 
        ...
        plt.annotate(f"Current Momentum: {latest_ema:.2f} CNY", 
