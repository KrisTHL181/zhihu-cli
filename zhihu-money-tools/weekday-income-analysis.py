...
def plot_weekday_analysis():
    try:
        ...
        ax = sns.boxplot(x='day_of_week', y='income_yuan', data=df, 
                         order=days_order, palette="Blues", showmeans=True,
                         meanprops={"marker":"o", "markerfacecolor":"white", "markeredgecolor":"black", "markersize":"8"})
        
        # Add a Swarm Plot on top to see individual data points (optional but helpful)
        sns.swarmplot(x='day_of_week', y='income_yuan', data=df, 
                      order=days_order, color=".25", size=4, alpha=0.6)
