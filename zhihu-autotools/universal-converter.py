...
def convert_items(items: List[Dict], forced_type: Optional[str] = None) -> List[Dict]:
...
def main():
    ...
    parser.add_argument(
        "inputs", nargs="+",
        help="Input JSON files (e.g., zhihu_user_activities.json, zhihu_articles.json, ...)"
    )
    parser.add_argument(
        "--output", "-o", default="all_assets_list.json",
        help="Output unified assets file (default: all_assets_list.json)"
    )
    parser.add_argument(
        "--type", "-t",
        help="Force a specific type for all items (e.g., 'article', 'answer', 'pin')"
    )
